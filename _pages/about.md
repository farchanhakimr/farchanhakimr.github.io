---
layout: about
title: about
permalink: /
subtitle: <a href="https://csie.ncu.edu.tw/" target="_blank">CSIE</a> at National Central University, Taiwan.

profile:
  align: right
  image: prof_pic.jpg
  image_circular: false # set to true if you want a round profile picture
  more_info: >
    <p>📧 farchan.hakim.r@g.ncu.edu.tw</p>
    <p>📍 Taoyuan, Taiwan</p>

news: false
selected_papers: false # We will list your 4 exact target publications manually below for precision control
social: true
---

## Short Bio
My name is **Farchan Hakim Raswa**. I am a Ph.D. Candidate in Computer Science and Information Engineering (CSIE) at **National Central University (NCU), Taiwan**. I am co-advised by [Prof. Jia-Ching Wang](https://dlmsl.csie.ncu.edu.tw/server) and [Prof. Chun-Shien Lu](https://homepage.iis.sinica.edu.tw/pages/lcs/index_en.html).

My research interests lie in the intersection of **Deep Learning** and **Computer Vision**, but are not limited to them, with a recently focus on Federated Learning, Medical Image Analysis, and Robust Biometric Security.

---

## Selected Publications

* **[Diffusion to Obfuscation: Time-Adaptive Synthesized Generation Against Gradient Leakage Attacks in Federated Learning](https://scholar.google.com/citations?user=U5iQ4b0AAAAJ&hl=en)**  
  **Farchan Hakim Raswa**, Chun-Shien Lu, Jia-Ching Wang  
  *European Conference on Computer Vision (ECCV)*, 2026 **(To-be appear)**

* **[Learning From Easy to Hard: Fingerprint Spoof Detection with Hard Sample Mining](https://ieeexplore.ieee.org/abstract/document/11389158/)**  
  Wenny Ramadha Putri\*, **Farchan Raswa Hakim\***, Bach-Tung Pham, Shang-Kuan Chen, Chung-I Huang, Kuo-Chen Li, Shih-Lun Chen, Jia-Ching Wang  
  *IEEE Transactions on Artificial Intelligence (IEEE TAI)*, 2026 **(Co-first author)**

* **[HistoFS: Non-IID Histopathologic Whole Slide Image Classification via Federated Style Transfer with RoI-Preserving](https://openaccess.thecvf.com/content/CVPR2025/html/Raswa_HistoFS_Non-IID_Histopathologic_Whole_Slide_Image_Classification_via_Federated_Style_CVPR_2025_paper.html)**  
  **Farchan Hakim Raswa**, Chun-Shien Lu, Jia-Ching Wang  
  *IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)*, 2025

* **[Attention-Guided Prototype Mixing: Diversifying Minority Context on Imbalanced Whole Slide Images Classification Learning](https://openaccess.thecvf.com/content/WACV2024/html/Raswa_Attention-Guided_Prototype_Mixing_Diversifying_Minority_Context_on_Imbalanced_Whole_Slide_WACV_2024_paper.html)**  
  **Farchan Hakim Raswa**, Chun-Shien Lu, Jia-Ching Wang  
  *IEEE/CVF Winter Conference on Applications of Computer Vision (WACV)*, 2024

---

## Community Service

* **Conference Reviewer:**
  * IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR)
  * European Conference on Computer Vision (ECCV)
  * Conference on Neural Information Processing Systems (NeurIPS)
* **Journal Reviewer:**
  * Frontiers
* **Volunteering:**
  * Student Volunteer at local engineering or computer science colloquiums.
